...
const buildMovieEntry = (movie: RadarrAllMedia, qualityProfiles: RadarrQualityProfiles) => {
...
export class RadarrClient extends ArrClient {
  ...
  public async getAllMedia(): Promise<Result<RadarrAllMedia[]>> {
    return this.makeRequest<RadarrAllMedia[]>("movie")
  }

  public async getAllQualityProfiles(): Promise<Result<RadarrQualityProfiles>> {
  ...
  public async getMediaData() {
    ...
    for (const movie of allMedia) {
      ...
      data.push(buildMovieEntry(movie, qualityProfiles))
    }
