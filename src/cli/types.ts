 ...
 *
 * In general, the data returned by radarr/sonarr may be `undefined`. Ultimately, the data returned by `getMediaData`
 * needs to be jsonifiable (no `undefined`), which we handle _after_ the data array is built via the
 * `normalizeMediaData` function.
 *
 ...
 *
 * Keys are optional because each client only produces its own subset of `AllMediaDataKeys`, e.g. a radarr data object
 * never has sonarr's `season`/`episode`/`type` keys.
 */
export type JsonifiableMediaData = SimplifyDeep<Partial<Record<AllMediaDataKeys, JsonPrimitive>>[]>
